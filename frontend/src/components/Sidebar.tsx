  ...
  return (
    <div className="space-y-8 animate-pulse p-2">
      <div className="border-b border-slate-100 pb-6">
        ...
        <div className="h-10 w-48 bg-slate-300 rounded-[2px] mb-4"></div>
      </div>
      <section className="space-y-4">
        <div className="h-5 w-40 bg-slate-200 rounded"></div>
        <div className="h-24 w-full bg-slate-100 rounded-[2px]"></div>
      </section>
      <section className="bg-slate-50 p-6 rounded-[2px] border border-slate-100 space-y-4">
...
const EmptyState = () => {
  return (
    <div className="flex flex-col items-center justify-center pt-16 pb-14 px-6 text-center">
      <div className="relative mb-8">
        
        <div className="relative bg-white p-5 rounded-[2px] shadow-none border border-green-100">
          <svg
      ...
      <h3 className="max-w-[250px] font-serif text-[32px] font-normal leading-[1.15] tracking-[-.6px] text-ink mb-3">
        Tout part de votre terrain.
      </h3>

      <p className="text-slate-600 text-sm leading-relaxed max-w-xs">
        Sélectionnez un point au Sénégal, puis lancez une
...
const ErrorMessage = ({ message, onRetry }: ErrorProps) => {
  return (
    <div className="p-6 bg-red-50 rounded-[2px] border border-red-100 text-center">
      <div className="w-12 h-12 bg-red-100 rounded-[2px] flex items-center justify-center mx-auto mb-4">
        <svg
      ...
      <h3 className="font-serif text-[23px] font-normal text-red-800 mb-2">
        Analyse impossible
      </h3>
      <p className="text-red-600 text-sm mb-6 leading-relaxed">{message}</p>
      <button
        onClick={onRetry}
        className="w-full py-3 bg-white border border-red-200 text-red-700 font-semibold rounded-[2px] hover:bg-red-100 transition-colors shadow-none"
      >
        Réessayer l'analyse
      </button>
    </div>
    ...
    return (
      <div ref={containerRef} className="relative h-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/60 backdrop-blur-[1px] ">
          <div className="bg-white p-6 rounded-[2px] shadow-none border border-slate-100 flex flex-col items-center gap-4 max-w-[80%] text-center">
  ...
  if (markerPosition && !recommendation) {
    return (
      <div ref={containerRef} className="flex flex-col items-center justify-center py-12 px-4 text-center">
        ...
        <h3 className="font-serif text-[23px] font-normal tracking-[-.6px] text-ink mb-2">
          {locationName || "Zone Sélectionnée"}
        </h3>
        <p className="text-slate-500 text-sm leading-relaxed max-w-60">
          Terrain sélectionné <br />
        ...
        <button
          ...
          className="flex gap-2 justify-center m-4 w-[85%] px-8 py-4 bg-green-600 text-white rounded-[2px] shadow-none hover:bg-green-800 transition-colors font-semibold text-base"
        >
    ...
    <div ref={containerRef} className={rootClassName}>
      <header className="border-b border-slate-100 pb-6 text-left pl-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <h1
            className={`font-serif font-normal tracking-[-.6px] text-ink leading-tight ${
              isPdfVariant ? "text-2xl" : "text-[28px] md:text-[30px]"
            }`}
          >
          Comprendre cette parcelle
          </h1>
          {!isPdfVariant && onDownloadPdf ? (
            <button
              onClick={onDownloadPdf}
              className="inline-flex items-center gap-2 rounded-[2px] border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-none hover:bg-slate-50"
            >
              <Download size={14} />
              PDF
            </button>
