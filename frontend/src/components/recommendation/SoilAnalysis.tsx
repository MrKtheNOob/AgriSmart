...
export default function SoilAnalysis({ soil, waterInsight }: SoilAnalysisProps) {
  ...
  return (
    <section className="bg-slate-50 p-6 rounded-none border border-slate-100 animate-in fade-in slide-in-from-bottom-4">
      <h3 className="text-slate-900 font-serif text-[23px] font-normal tracking-[-.6px] mb-6 flex items-center gap-2">
        Profil du Sol ({soil.classification || "Zone Aride"})
      </h3>

      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="bg-white p-4 rounded-[2px] border border-slate-100 shadow-none">
          <span className="block text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">pH du Sol</span>
          <span className="text-xl font-semibold tabular-nums text-slate-800">{getSoilProp("pH")}</span>
        </div>

        <div className="bg-white p-4 rounded-[2px] border border-slate-100 shadow-none">
          <span className="block text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Carbone Organique</span>
          <span className="text-xl font-semibold tabular-nums text-slate-800">{getSoilProp("Carbon, organic")}</span>
        </div>
      ...
      <div className="mb-8">
        <h4 className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold tabular-nums mb-4">Fertilité & Nutriments</h4>

        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="bg-white p-4 rounded-[2px] border border-slate-100 text-center shadow-none">
            <span className="block text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Azote (N)</span>
            <span className="text-base font-semibold tabular-nums text-slate-800">{getSoilProp("Nitrogen")}</span>
          </div>

          <div className="bg-white p-4 rounded-[2px] border border-slate-100 text-center shadow-none">
            <span className="block text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Phosphore (P)</span>
            <span className="text-base font-semibold tabular-nums text-slate-800">{getSoilProp("Phosphorus")}</span>
          </div>

          <div className="bg-white p-4 rounded-[2px] border border-slate-100 text-center shadow-none">
            <span className="block text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Potassium (K)</span>
            <span className="text-base font-semibold tabular-nums text-slate-800">{getSoilProp("Potassium")}</span>
          </div>
        </div>

        <div className="bg-white p-4 rounded-[2px] border border-slate-100 flex justify-between items-center shadow-none">
          <span className="text-sm font-medium text-slate-600">Capacité d&apos;Échange (CEC)</span>
          <span className="text-base font-semibold tabular-nums text-slate-800">{getSoilProp("Cation Exchange Capacity")}</span>
        </div>
      </div>

      {waterInsight ? (
        <div className="mb-8 bg-blue-50/50 p-5 rounded-[2px] border border-blue-100/50">
          <h4 className="text-[11px] uppercase tracking-widest text-blue-600 font-semibold tabular-nums mb-4">Analyse Hydrique</h4>
          <div className="flex justify-between items-end mb-2">
            <span className="text-sm font-bold text-slate-700">Rétention d&apos;eau</span>
            <span className="text-2xl font-semibold tabular-nums text-blue-700">{waterInsight.retention_score}%</span>
          </div>
          <div className="w-full bg-blue-200/30 rounded-[2px] h-2 mb-4">
            <div
              className="bg-blue-500 h-2 rounded-[2px]"
              style={{ width: `${waterInsight.retention_score}%` }}
            ></div>
          </div>
          <div className="bg-white/80 backdrop-blur-sm p-3 rounded-[2px] border border-blue-100">
            <p className="text-[10px] font-semibold tabular-nums uppercase text-blue-500 mb-1">{waterInsight.category}</p>
      ...
      {(getSoilProp("Stone content") !== "N/A" || getSoilProp("Depth to bedrock") !== "N/A") && (
        <div>
          <h4 className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold tabular-nums mb-4">Détails Techniques</h4>
          <div className="bg-white p-4 rounded-[2px] border border-slate-100 shadow-none space-y-3">
