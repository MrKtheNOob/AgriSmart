...
4. For each crop:
   ...
   - Prefer crops explicitly supported by the retrieved knowledge base context
   - Do not introduce crops unless they are clearly justified by the context and conditions

Strict Rules:
- Use only the provided data.
- Do NOT invent environmental metrics.
- Do NOT assume irrigation unless explicitly stated.
- Do NOT use crops that are not supported by the retrieved context or by the conditions described.
...
class RAGService:
    def __init__(
        ...
        # Profitability data is temporarily disabled until the source dataset is available.
        self.profit_data = None
        if profit_data_path:
            logger.info("Profit data path provided, but profitability context is disabled for now.")
    ...
    async def generate_recommendation(self, soil_data: SoilProfile, climate_data: ClimateMetrics):
        ...
        try:
            ...
            # Profitability context is intentionally omitted for now.
            context = {
                "retrieved_docs": context,
            }
            logger.info(f"Context for LLM: {context}")
